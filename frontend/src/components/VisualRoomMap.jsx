                ...
                {beds.map((b, idx) => (
                  <div
                    ...
                    className={`p-3 rounded-2xl border text-xs transition-all ${
                      b.patient
                        ? 'bg-cyan-500/10 dark:bg-cyan-950/40 border-cyan-500/40 text-body shadow-sm'
                        : 'bg-emerald-500/5 dark:bg-emerald-950/10 border-emerald-500/25 border-dashed text-muted hover:border-emerald-400/50'
                    }`}
                  >
                    <div className="flex items-center justify-between font-bold mb-1.5">
                      <span className="flex items-center gap-1.5 text-xs text-body">
                        <Bed className={`h-4 w-4 ${b.patient ? 'text-cyan-500 dark:text-cyan-400' : 'text-emerald-500 dark:text-emerald-400'}`} />
                            ...
                            <ActionMenu
                              ...
                              items={[
                                ...
                                },
                                {
                                  ...
                                  variant: 'gold',
                                  onClick: () => onPrintReceipt && onPrintReceipt(b.patient),
                                },
                                {
                                  label: "🚪 Bemor chiqarish (Выписка)",
                                  variant: 'gold',
                                  onClick: () => onDischarge && onDischarge(b.patient),
                                },
                              ]}
                            />
                          </>
                        ) : (
                          <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-extrabold border border-emerald-500/40">
                            BO'SH
                          </span>
                        )}
                      </div>
                    ...
                    {b.patient ? (
                      <div className="space-y-2 pt-0.5">
                        <button
                          type="button"
                          onClick={() => onViewPatient && onViewPatient(b.patient)}
                          className="font-extrabold text-sm text-cyan-600 dark:text-cyan-300 hover:underline text-left block w-full truncate"
                          title="Bemor tibbiy kartasini ko'rish"
                        >
                          {b.patient.first_name} {b.patient.last_name}
                        </button>
                        
                        <div className="text-[10px] font-mono space-y-0.5 p-2 bg-white/80 dark:bg-surface-2/60 rounded-xl border border-border/80 text-body">
                          <div className="flex justify-between text-muted">
                            <span>Jami: {formatMoney(b.patient.total_amount)}</span>
                            <span className="text-emerald-600 dark:text-emerald-400 font-bold">To'langan: {formatMoney(b.patient.paid_total)}</span>
                          </div>
                          <div className="flex justify-between items-center font-bold">
                            <span className="text-muted">Qoldiq (Qarz):</span>
                            <span className={b.patient.balance_due > 0 ? 'text-rose-600 dark:text-rose-400 font-black' : 'text-emerald-600 dark:text-emerald-400 font-black'}>
                        ...
                        <div className="pt-0.5 flex gap-1.5">
                          <button
                            type="button"
                            onClick={() => onPay && onPay(b.patient)}
                            className="btn-outline flex-1 border-emerald-500/40 text-emerald-600 dark:text-emerald-300 hover:bg-emerald-500/15 py-1 px-2 text-xs font-bold flex items-center justify-center gap-1 rounded-xl"
                          ...
                          <button
                            type="button"
                            onClick={() => onDischarge && onDischarge(b.patient)}
                            className="btn-gold flex-1 py-1 px-2 text-xs font-bold flex items-center justify-center gap-1 rounded-xl"
                      ...
                      <div className="pt-2 text-center">
                        <button
                          ...
                          className="btn-outline border-emerald-500/40 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 w-full py-1 text-[11px] font-bold"
                        >
                          + Bemor qabul qilish
                        </button>
                      </div>
